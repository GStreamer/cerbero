...
class StaticFrameworkLibrary(FrameworkLibrary):
    ...
    def _split_static_lib(self, lib, thin_arch=None):
        ...
        shell.call('ar -x %s' % tmplib, lib_tmpdir, env=self.env)
        ...
        files = shell.check_call('ar -t %s' % tmplib, lib_tmpdir, env=self.env).split('\n')
        ...
        for f in dups:
            ...
            for x in range(dups[f]):
                ...
                shell.call('ar -x %s %s' % (tmplib, f), lib_tmpdir, env=self.env)
                shutil.move (path, new_path)
                shell.call('ar -d %s %s' % (tmplib, f), lib_tmpdir, env=self.env)
    ...
    def _check_duplicated_symbols(self, files, tmpdir):
        for f in files:
            ...
            symbols = shell.check_call('nm -UA %s' % f, tmpdir, env=self.env).split('\n')
    ...
    def _create_framework_library(self, libraries):
        ...
        for thin_arch in archs:
            ...
            for lib in libraries:
                ...
                for obj_f in obj_files:
                    ...
                    md5 = shell.check_call('md5 -q %s' % obj_path, env=self.env).split('\n')[0]
                    ...
                    if md5 not in object_files_md5:
                        ...
                        try:
                            ...
                            shell.call('cp %s %s' % (obj_path, target_name), tmpdir_thinarch, env=self.env)
                        ...
                        if target_name in target_objs:
                            ...
                            shell.call('ar -cqS %s %s' % (libname, " ".join (target_objs)), tmpdir_thinarch, env=self.env)
                            target_objs = []
            ...
            shell.call('ar -s %s' % (libname), tmpdir_thinarch, env=self.env)
        ...
        if len(archs) > 1:
            ...
            shell.call('lipo %s -create -output %s' % (' '.join(files), self.install_name), tmpdir, env=self.env)
        else:
            shell.call('cp %s %s' % (os.path.join(tmpdir, self.arch, libname), self.install_name), tmpdir, env=self.env)
