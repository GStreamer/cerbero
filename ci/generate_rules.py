...
### END NINJA-BUILD CODE ###

# ruff: noqa: E402
import os

if __name__ == '__main__':
    rules = open('build.ninja', 'w', encoding='utf-8')
    writer = Writer(rules, width=120)

    writer.comment('This is the build file for the Android examples CI.')
    ...
    writer.rule(
        'run_retry',
        '$in --project-dir $projectdir --no-daemon --parallel --profile $target && cp $artifact ./',
        description='Building Gradle target $target in $projectdir',
    )

    writer.newline()
    writer.newline()
    projectdir = f"{os.environ['EXAMPLES_HOME']}/gst-examples/playback/player/android"
    writer.build(
        'app-debug.apk',
        'run_retry',
        f'{projectdir}/gradlew',
        variables={
            'projectdir': projectdir,
            'target': 'assembleDebug',
            'artifact': f'{projectdir}/app/build/outputs/apk/debug/*.apk',
        },
    )

    writer.newline()
    projectdir = f"{os.environ['EXAMPLES_HOME']}/gst-examples/vulkan/android"
    writer.build(
        'org.freedesktop.gstreamer.vulkan.vulkan_1-v1-debug.apk',
        'run_retry',
        f'{projectdir}/gradlew',
        variables={
            'projectdir': projectdir,
            'target': 'assembleDebug',
            'artifact': f'{projectdir}/build/outputs/apk/debug/*.apk',
        },
    )

    out = [
        'android-tutorial-1-debug.apk',
        'android-tutorial-2-debug.apk',
        'android-tutorial-3-debug.apk',
        'android-tutorial-4-debug.apk',
        'android-tutorial-5-debug.apk',
    ]
    writer.newline()
    projectdir = f"{os.environ['EXAMPLES_HOME']}/gst-docs/examples/tutorials/android"
    writer.build(
        out,
        'run_retry',
        f'{projectdir}/gradlew',
        variables={
            'projectdir': projectdir,
            'target': 'assembleDebug',
            'artifact': f'{projectdir}/android-tutorial-*/build/outputs/apk/debug/*.apk',
        },
    )
